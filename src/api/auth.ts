...
export function useAuth() {
  ...
  async function login(token: string): Promise<void> {
    ...
    if (!res.ok) {
      if (res.status === 401) {
        throw new AuthError('Invalid token. Check `RELAY_ADMIN_TOKEN` on your relay deployment.')
      }
      throw new Error(`Login failed: ${res.status}`)
    }
    await queryClient.invalidateQueries({ queryKey: whoamiQueryOptions.queryKey })
  }

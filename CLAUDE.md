...
- **One loud action per view.** `Button`'s `default` rung is the quiet everyday affirmative (tonal wash, never screams); `variant="cta"` is the loud solid, rationed to the single hero action (Create/Save/Attach). Everything else stays quieter still (outline/ghost). Don't hand-roll CTA styling with brand colors.
